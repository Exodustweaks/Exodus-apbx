...
set "AtmosphereOther=%windir%\AtmosphereModules\Other"
set "winServices=%AtmosphereOther%\winServices.reg"
set "AtmosphereServices=%AtmosphereOther%\AtmosphereServices.reg"
if exist "%winServices%" (
	if exist "%AtmosphereServices%" call :fullRestore
)
...
echo 2) Restore a full services backup of the default Atmosphere services
echo 3) Nothing
echo]
choice /c:123 /n /m "Choose a number: [1/2/3] "
if "%errorlevel%"=="1" reg import "%winServices%" > nul
if "%errorlevel%"=="2" reg import "%AtmosphereServices%" > nul
exit /b

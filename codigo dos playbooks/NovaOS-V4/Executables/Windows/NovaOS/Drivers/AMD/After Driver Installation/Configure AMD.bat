...
if %errorlevel% equ 1 (
	...
	powershell -NoProfile -ExecutionPolicy Bypass -Command "Get-PnpDevice -FriendlyName 'AMD Controller Emulation' -ErrorAction SilentlyContinue | Disable-PnpDevice -Confirm:$false"
	powershell -NoProfile -ExecutionPolicy Bypass -Command "Get-PnpDevice -FriendlyName 'AMD Crash Defender' -ErrorAction SilentlyContinue | Disable-PnpDevice -Confirm:$false"
	exit
)
if %errorlevel% equ 2 (
	@echo Revert Sleep
	...
	@echo Revert Spread Spectrum
	reg delete "%gpu_key%" /v "EnableSpreadSpectrum" /f > nul 2>&1

	@echo Revert Power Saver
	reg add "HKCU\Software\AMD\CN" /v "PowerSaverAutoEnable_CUR" /t REG_DWORD /d "1" /f > nul 2>&1

	@echo Revert Services
	reg add "HKLM\System\CurrentControlSet\Services\amdwddmg" /v "ChillEnabled" /t REG_DWORD /d "1" /f > nul 2>&1
	reg add "HKLM\System\CurrentControlSet\Services\AMD Crash Defender Service" /v "Start" /t REG_DWORD /d "2" /f > nul 2>&1
	reg add "HKLM\System\CurrentControlSet\Services\AMD External Events Utility" /v "Start" /t REG_DWORD /d "2" /f > nul 2>&1
	reg add "HKLM\System\CurrentControlSet\Services\amdfendr" /v "Start" /t REG_DWORD /d "2" /f > nul 2>&1
	reg add "HKLM\System\CurrentControlSet\Services\amdfendrmgr" /v "Start" /t REG_DWORD /d "2" /f > nul 2>&1
	reg add "HKLM\System\CurrentControlSet\Services\amdlog" /v "Start" /t REG_DWORD /d "2" /f > nul 2>&1

	@echo Revert Auto Update
	reg add "HKCU\Software\AMD\CN" /v "AutoUpdateTriggered" /t REG_DWORD /d "1" /f > nul 2>&1
	reg add "HKCU\Software\AMD\CN" /v "AutoUpdate" /t REG_DWORD /d "1" /f > nul 2>&1

	@echo Revert Animations
	reg add "HKCU\Software\AMD\CN" /v "AnimationEffect" /t REG_SZ /d "true" /f > nul 2>&1
	sc config amdlog start=auto

	@echo Revert Devices
	powershell -NoProfile -ExecutionPolicy Bypass -Command "Get-PnpDevice -FriendlyName 'AMD Controller Emulation' -ErrorAction SilentlyContinue | Enable-PnpDevice -Confirm:$false" > nul 2>&1
	powershell -NoProfile -ExecutionPolicy Bypass -Command "Get-PnpDevice -FriendlyName 'AMD Crash Defender' -ErrorAction SilentlyContinue | Enable-PnpDevice -Confirm:$false" > nul 2>&1
	exit
)